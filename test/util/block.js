...
//
// handles only `sof_getBlockByNumber` requests
...
function createBlock(blockParams, prevBlock, txs) {
  ...
  var defaultGasLimit = sofUtil.intToHex(4712388)
  return extend({
    // defaults
...
function incrementHex(hexString){
  return stripLeadingZeroes(sofUtil.intToHex(Number(hexString)+1))
}

function randomHash(){
  return sofUtil.bufferToHex(sofUtil.toBuffer(Math.floor(Math.random()*Number.MAX_SAFE_INTEGER)))
}

function stripLeadingZeroes (hexString) {
  let strippedHex = sofUtil.stripHexPrefix(hexString)
  while (strippedHex[0] === '0') {
    strippedHex = strippedHex.substr(1)
  }
  return sofUtil.addHexPrefix(strippedHex)
}
