...
const HookedWalletTxProvider = require('../subproviders/hooked-wallet-softx.js')
...
test('tx sig', function(t){
  ...
  var providerC = injectMetrics(new FixtureProvider({
    sof_gasPrice: '0x1234',
    sof_getTransactionCount: '0x00',
    sof_sendRawTransaction: function(payload, next, done){
      var rawTx = sofUtil.toBuffer(payload.params[0])
  ...
  engine.sendAsync(createPayload(txPayload), function(err, response){
    ...
    // intial tx request
    t.equal(providerA.getWitnessed('sof_sendTransaction').length, 1, 'providerA did see "signTransaction"')
    t.equal(providerA.getHandled('sof_sendTransaction').length, 1, 'providerA did handle "signTransaction"')

    // tx nonce
    t.equal(providerB.getWitnessed('sof_getTransactionCount').length, 1, 'providerB did see "sof_getTransactionCount"')
    t.equal(providerB.getHandled('sof_getTransactionCount').length, 0, 'providerB did NOT handle "sof_getTransactionCount"')
    t.equal(providerC.getWitnessed('sof_getTransactionCount').length, 1, 'providerC did see "sof_getTransactionCount"')
    t.equal(providerC.getHandled('sof_getTransactionCount').length, 1, 'providerC did handle "sof_getTransactionCount"')

    // gas price
    t.equal(providerC.getWitnessed('sof_gasPrice').length, 1, 'providerB did see "sof_gasPrice"')
    t.equal(providerC.getHandled('sof_gasPrice').length, 1, 'providerB did handle "sof_gasPrice"')

    // send raw tx
    t.equal(providerC.getWitnessed('sof_sendRawTransaction').length, 1, 'providerC did see "sof_sendRawTransaction"')
    t.equal(providerC.getHandled('sof_sendRawTransaction').length, 1, 'providerC did handle "sof_sendRawTransaction"')
...
// so I made a script out of graviton's internals to create this test data
...
signatureTest({
  ...
  method: 'personal_sign',
  // some random binary message from susy's test
...
signatureTest({
  ...
  method: 'personal_sign',
  // random binary message data and pk from susy's test
  // https://github.com/sofcore/susy/blob/5369a129ae276d38f3490abb18c5093b338246e0/rpc/src/v1/tests/mocked/sof.rs#L301-L317
  // note: their signature result is incorrect (last byte moved to front) due to a susy bug
  message: '0x0cc175b9c0f1b6a831c399e26977266192eb5ffee6ae2fec3ad71c777531578f',
...
recoverTest({
  testLabel: 'graviton kumavis manual I recover',
...
recoverTest({
  testLabel: 'graviton kumavis manual II recover',
  method: 'personal_ecRecover',
  // message from susy's test - note result is different than what they are testing against
  // https://github.com/sofcore/susy/blob/5369a129ae276d38f3490abb18c5093b338246e0/rpc/src/v1/tests/mocked/sof.rs#L301-L317
  message: '0x0cc175b9c0f1b6a831c399e26977266192eb5ffee6ae2fec3ad71c777531578f',
  signature: '0x9ff8350cc7354b80740a3580d0e0fd4f1f02062040bc06b893d70906f8728bb5163837fd376bf77ce03b55e9bd092b32af60e86abce48f7b8d3539988ee5a9be1c',
  addressHex: '0xbe93f9bacbcffc8ee6663f2647917ed7a20a57bb',
