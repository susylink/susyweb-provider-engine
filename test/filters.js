...
filterTest('basic block filter', { method: 'sof_newBlockFilter' },
...
filterTest('sof_getFilterLogs called with non log filter id should return []', { method: 'sof_newBlockFilter' },
  function afterInstall(t, testMeta, response, cb){
    var block = testMeta.block = testMeta.blockProvider.nextBlock()
    testMeta.engine.once('block', function(){
      testMeta.engine.sendAsync(createPayload({ method: 'sof_getFilterLogs', params: [testMeta.filterId] }), function(err, response){
        t.ifError(err, 'did not error')
        t.ok(response, 'has response')
        t.ok(response.result, 'has response.result')

        t.equal(testMeta.filterProvider.getWitnessed('sof_getFilterLogs').length, 1, 'filterProvider did see "sof_getFilterLogs')
        t.equal(testMeta.filterProvider.getHandled('sof_getFilterLogs').length, 1, 'filterProvider did handle "sof_getFilterLogs')


        t.equal(response.result.length, 0, 'sof_getFilterLogs returned an empty result for a non log filter')
        cb()
      })
    })
  })
...
function filterTest(label, filterPayload, afterInstall, filterChangesOne, filterChangesTwo){
  test('filters - '+label, function(t){
    ...
    function continueTest(filterId){
      ...
      engine.sendAsync(createPayload({ method: 'sof_getFilterChanges', params: [filterId] }), function(err, response){
        t.ifError(err, 'did not error')
        t.ok(response, 'has response')

        t.equal(filterProvider.getWitnessed('sof_getFilterChanges').length, 1, 'filterProvider did see "sof_getFilterChanges"')
        t.equal(filterProvider.getHandled('sof_getFilterChanges').length, 1, 'filterProvider did handle "sof_getFilterChanges"')
        ...
        filterChangesOne(t, testMeta, response, function(err){
          ...
          if (filterChangesTwo){
            engine.sendAsync(createPayload({ method: 'sof_getFilterChanges', params: [filterId] }), function(err, response){
              ...
              t.equal(filterProvider.getWitnessed('sof_getFilterChanges').length, 2, 'filterProvider did see "sof_getFilterChanges"')
              t.equal(filterProvider.getHandled('sof_getFilterChanges').length, 2, 'filterProvider did handle "sof_getFilterChanges"')
