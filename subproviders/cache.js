...
function BlockCacheProvider(opts) {
  ...
  self.strategies = {
    perma: new ConditionalPermaCacheStrategy({
      sof_getTransactionByHash: containsBlockhash,
      sof_getTransactionReceipt: containsBlockhash,
    }),
...
BlockCacheProvider.prototype.handleRequest = function(payload, next, end){
  ...
  if (payload.method === 'sof_getBlockByNumber' && payload.params[0] === 'latest') {
...
BlockCacheProvider.prototype._handleRequest = function(payload, next, end){
  ...
  if (blockTag === 'earliest') {
    ...
    requestedBlockNumber = sofUtil.bufferToHex(self.currentBlock.number)
  } else {
...
BlockCacheStrategy.prototype.cacheRollOff = function(previousBlock){
  const self = this
  const previousHex = sofUtil.bufferToHex(previousBlock.number)
...
function hexToBN(hex){
  return new BN(sofUtil.toBuffer(hex))
}
