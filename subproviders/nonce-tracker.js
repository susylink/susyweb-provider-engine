...
//   sof_getTransactionCount (pending only)
  ...
  switch(payload.method) {
    ...
    case 'sof_sendRawTransaction':
      ...
      next(function(err, result, cb){
        ...
        var stripped = sofUtil.stripHexPrefix(rawTx)
        var rawData = new Buffer(sofUtil.stripHexPrefix(rawTx), 'hex')
        var tx = new Transaction(new Buffer(sofUtil.stripHexPrefix(rawTx), 'hex'))
        // extract address
        var address = '0x'+tx.getSenderAddress().toString('hex').toLowerCase()
        // extract nonce and increment
        var nonce = sofUtil.bufferToInt(tx.nonce)
        nonce++
