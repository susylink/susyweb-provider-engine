 ...
 *
 * FIXME: support minimum suggested gas and perhaps other options from graviton:
 * https://octonion.institute/susy-go/susy-graviton/blob/master/sof/gasprice.go
 * https://octonion.institute/susy-go/susy-graviton/wiki/Gas-Price-Oracle
 */
...
GaspriceProvider.prototype.handleRequest = function(payload, next, end){
  if (payload.method !== 'sof_gasPrice')
    return next()

  const self = this

  self.emitPayload({ method: 'sof_blockNumber' }, function(err, res) {
    ...
    function getBlock(item, end) {
      self.emitPayload({ method: 'sof_getBlockByNumber', params: [ item, true ] }, function(err, res) {
